/**
 * FPI Pattern Uniqueness Self-Check
 * ----------------------------------
 * Verifies that the 20 NORMAL_TYPES pattern vectors in
 * `src/data/fpi/types.ts` are all unique AND that the minimum
 * Hamming distance between any two patterns is >= 2.
 *
 * Rationale: matching logic relies on each type having a distinct
 * 6-axis fingerprint. A min Hamming of 2 means any single-axis
 * noise in a user's answers cannot collapse one type into another.
 *
 * Run:
 *   npx tsx scripts/fpi-pattern-check.ts
 *
 * Exits non-zero on failure so the script can gate CI if desired.
 */

import { NORMAL_TYPES } from '../src/data/fpi/types';
