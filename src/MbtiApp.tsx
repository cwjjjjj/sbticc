...
function MbtiHero({ onStartTest, totalTests }: { onStartTest: () => void; totalTests: number }) {
...
/* ---------- Mbti Nav (simpler: only home + ranking) ---------- */

type MbtiTabId = 'home' | 'ranking';

const mbtiTabs: { id: MbtiTabId; label: string }[] = [
  { id: 'home', label: '首页' },
...
function MbtiAppInner() {
  const config = useTestConfig();
  const [activeTab, setActiveTab] = useState<MbtiTabId>('home');
  ...
  const [shareModalFileName, setShareModalFileName] = useState('mbti-share.png');
  ...
  // Adapt MbtiTabId to Nav's TabId
  const handleTabChange = useCallback((tab: TabId) => {
    if (tab === 'home' || tab === 'ranking') {
      setActiveTab(tab as MbtiTabId);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-bg text-white font-sans">
      ...
      {!showOverlay && (
        <main>
          {activeTab === 'home' && (
            <MbtiHero onStartTest={handleStartTest} totalTests={totalTests} />
          )}
          {activeTab === 'ranking' && (
            <RankingPage
              ranking={ranking}
