...
import { zhtiConfig } from './data/zhti/config';
...
function ZhtiHero({ onStartTest, totalTests }: { onStartTest: () => void; totalTests: number }) {
    ...
    >
      ...
      <motion.div
        ...
        style={{
          width: 60,
          height: 3,
          background: 'linear-gradient(90deg, #c8943a, #e8c878)',
        }}
      />
      ...
      >
        18道宫廷情境题 &times; 16个人物 — 你在深宫里是哪种人？
      </motion.p>
...
type ZhtiTabId = 'home' | 'profiles' | 'ranking';

function ZhtiAppInner() {
  const config = useTestConfig();
  const [activeTab, setActiveTab] = useState<ZhtiTabId>('home');
  ...
  const [shareModalFileName, setShareModalFileName] = useState('zhti-share.png');
    ...
    <div className="min-h-screen bg-bg text-white font-sans">
      {!showOverlay && (
        <Nav
          ...
          tabs={[{ id: 'home', label: '首页' }, { id: 'profiles', label: '人物介绍' }, { id: 'ranking', label: '全站排行' }]}
        />
      )}
      ...
      {!showOverlay && (
        <main>
          {activeTab === 'home' && (
            <ZhtiHero onStartTest={handleStartTest} totalTests={totalTests} />
          )}
          {activeTab === 'profiles' && (
            <div className="pt-28">
...
export default function ZhtiApp() {
  return (
    <TestConfigProvider config={zhtiConfig}>
      <ZhtiAppInner />
    </TestConfigProvider>
  );
}
