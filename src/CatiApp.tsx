...
import { catiConfig } from './data/cati/config';
...
function CatiHero({ onStartTest, totalTests }: { onStartTest: () => void; totalTests: number }) {
  ...
  return (
    ...
    >
      ...
      >
        12 题 × 16 种猫猫人格 — 把你的 MBTI 翻译成猫咪气质
      </motion.p>
...
type CatiTabId = 'home' | 'profiles' | 'ranking';

/* ---------- CatiAppInner ---------- */

function CatiAppInner() {
  const config = useTestConfig();
  const [activeTab, setActiveTab] = useState<CatiTabId>('home');
  ...
  const [shareModalFileName, setShareModalFileName] = useState('cati-share.png');
  ...
  // Adapt CatiTabId to Nav's TabId
  const handleTabChange = useCallback((tab: TabId) => {
    if (tab === 'home' || tab === 'profiles' || tab === 'ranking') {
      setActiveTab(tab as CatiTabId);
    }
  }, []);
    ...
    <div className="min-h-screen bg-bg text-white font-sans">
      ...
      {!showOverlay && (
        <Nav
          ...
          tabs={[{ id: "home", label: "首页" }, { id: "profiles", label: "猫猫图鉴" }, { id: "ranking", label: "全站排行" }]}
        />
      )}
      ...
      {!showOverlay && (
        <main>
          {activeTab === 'home' && (
            <CatiHero onStartTest={handleStartTest} totalTests={totalTests} />
          )}
          {activeTab === 'profiles' && (
            <div className="pt-28">
...
export default function CatiApp() {
  return (
    <TestConfigProvider config={catiConfig}>
      <CatiAppInner />
    </TestConfigProvider>
  );
}
