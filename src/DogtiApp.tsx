...
const heroGlow = css`
  background: radial-gradient(
    ...
    rgba(255, 170, 80, 0.08) 0%,
    transparent 70%
  );
`;

function DogtiHero({ onStartTest, totalTests }: { onStartTest: () => void; totalTests: number }) {
  ...
  return (
    ...
    >
      ...
      >
        12 题 × 16 种狗狗人格 — 把你的 MBTI 翻译成狗狗气质
      </motion.p>
...
type DogtiTabId = 'home' | 'profiles' | 'ranking';

/* ---------- DogtiAppInner ---------- */

function DogtiAppInner() {
  const config = useTestConfig();
  const [activeTab, setActiveTab] = useState<DogtiTabId>('home');
  ...
  const [shareModalFileName, setShareModalFileName] = useState('dogti-share.png');
  ...
  // Adapt DogtiTabId to Nav's TabId
  const handleTabChange = useCallback((tab: TabId) => {
    if (tab === 'home' || tab === 'profiles' || tab === 'ranking') {
      setActiveTab(tab as DogtiTabId);
    }
  }, []);
    ...
    <div className="min-h-screen bg-bg text-white font-sans">
      ...
      {!showOverlay && (
        <Nav
          ...
          tabs={[{ id: "home", label: "首页" }, { id: "profiles", label: "狗狗图鉴" }, { id: "ranking", label: "全站排行" }]}
        />
      )}
      ...
      {!showOverlay && (
        <main>
          {activeTab === 'home' && (
            <DogtiHero onStartTest={handleStartTest} totalTests={totalTests} />
          )}
          {activeTab === 'profiles' && (
            <div className="pt-28">
...
export default function DogtiApp() {
  return (
    <TestConfigProvider config={dogtiConfig}>
      <DogtiAppInner />
    </TestConfigProvider>
  );
}
