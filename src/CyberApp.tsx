...
function CyberHero({ onStartTest, totalTests }: { onStartTest: () => void; totalTests: number }) {
  ...
  return (
    ...
    >
      ...
      >
        4个维度 &times; 16种网络人格 — 你在互联网上是什么物种？
      </motion.p>
...
type CyberTabId = 'home' | 'profiles' | 'ranking';

/* ---------- CyberAppInner ---------- */

function CyberAppInner() {
  const config = useTestConfig();
  const [activeTab, setActiveTab] = useState<CyberTabId>('home');
  ...
  const [shareModalFileName, setShareModalFileName] = useState('cyber-share.png');
  ...
  // Adapt CyberTabId to Nav's TabId
  const handleTabChange = useCallback((tab: TabId) => {
    if (tab === 'home' || tab === 'profiles' || tab === 'ranking') {
      setActiveTab(tab as CyberTabId);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-bg text-white font-sans">
      ...
      {!showOverlay && (
        <main>
          {activeTab === 'home' && (
            <CyberHero onStartTest={handleStartTest} totalTests={totalTests} />
          )}
          {activeTab === 'profiles' && (
            <div className="pt-28">
...
export default function CyberApp() {
  return (
    <TestConfigProvider config={cyberConfig}>
      <CyberAppInner />
    </TestConfigProvider>
  );
}
