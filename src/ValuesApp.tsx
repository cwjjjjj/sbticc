...
import { valuesConfig } from './data/values/config';
...
function ValuesHero({ onStartTest, totalTests }: { onStartTest: () => void; totalTests: number }) {
...
/* ---------- Values Nav (simpler: only home + ranking) ---------- */

type ValuesTabId = 'home' | 'profiles' | 'ranking';

const valuesTabs: { id: ValuesTabId; label: string }[] = [
  { id: 'home', label: '首页' },
...
function ValuesAppInner() {
  const config = useTestConfig();
  const [activeTab, setActiveTab] = useState<ValuesTabId>('home');
  ...
  const [shareModalFileName, setShareModalFileName] = useState('values-share.png');
  ...
  // Adapt ValuesTabId to Nav's TabId
  const handleTabChange = useCallback((tab: TabId) => {
    if (tab === 'home' || tab === 'profiles' || tab === 'ranking') {
      setActiveTab(tab as ValuesTabId);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-bg text-white font-sans">
      ...
      {!showOverlay && (
        <main>
          {activeTab === 'home' && (
            <ValuesHero onStartTest={handleStartTest} totalTests={totalTests} />
          )}
          {activeTab === 'profiles' && (
            <div className="pt-28">
...
export default function ValuesApp() {
  return (
    <TestConfigProvider config={valuesConfig}>
      <ValuesAppInner />
    </TestConfigProvider>
  );
}
