...
function WorkHero({ onStartTest, totalTests }: { onStartTest: () => void; totalTests: number }) {
...
function WorkAppInner() {
  const config = useTestConfig();
  const [activeTab, setActiveTab] = useState<WorkTabId>('home');
  ...
  const [shareModalFileName, setShareModalFileName] = useState('work-share.png');
...
export default function WorkApp() {
  return (
    <TestConfigProvider config={workConfig}>
      <WorkAppInner />
    </TestConfigProvider>
  );
}
